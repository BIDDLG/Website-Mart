...
function showResults(query) {
  ...
  allFiles.forEach(file => {
    if (file.toLowerCase().includes(query.toLowerCase())) {
      ...
      const fileUrl = `projects/${file}.txt`;

      card.innerHTML = `    
        <span>${title}</span>    
        <button class="copyBtn" data-url="${fileUrl}" title="Copy Link">    
  ...
  // Bind copy functionality after results loaded
