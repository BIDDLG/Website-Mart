...
const apiUrl = "https://gplinks.in/api";
...
async function getShortenedUrl(originalUrl) {
  try {
    const response = await fetch(`${apiUrl}?api=${apiKey}&url=${encodeURIComponent(originalUrl)}`);
...
async function showResults(query) {  
  ...
  for (const file of allFiles) {
    if (file.toLowerCase().includes(query.toLowerCase())) {
      ...
      const directUrl = `https://biddlg.github.io/Website-Mart/projects/${file}.txt`;
      const shortUrl = await getShortenedUrl(directUrl);
      ...
      card.innerHTML = `  
        <span>${title}</span>  
        <a href="${shortUrl}" target="_blank" title="Download">  
...
// Block long-press on links (mostly on mobile)
